...
def signup():
    ...
    if request.method == "POST":
        ...
        if acnt:
            ...
            mg = Successfully registered. Click login to proceed!'
            message = Mail(
                from_email=os.environ.get('MAIL_DEFAULT_SENDER'),
                to_emails=email,
                subject='New SignUp',
                html_content='<p>Hello, You have  successfully registered with us. <br><br> Thank you for choosing us.</p>')
        ...
        msg = "Fill the form to SignUp with us!"
...
def dashBoard():
    ...
    return render_template("dashboard.html", headings=headings, data=stock_details)
...
def deleteStocks():
    if request.method == "POST":
        ...
        try:
            item = request.form['item']
            insert_sql = 'DELETE FROM stock-details WHERE Name=?'
            pstmt = ibm_db.prepare(conn, insert_sql)
            ibm_db.bind_param(pstmt, 1, item)
            ibm_db.execute(pstmt)
        except Exception as e:
    ...
    if request.method == "POST":
        try:
            ...
            if dictionary:
                ...
                query = 'INSERT INTO Order_details(Stocks_id,Quantity,Date,Delivery_date,Price) VALUES (?,?,?,?,?)'
...
def UpdateSupplier():
    if request.method == "POST":
        try:
            ...
            value = request.form['input-value']
            print(item, field, value)
            insert_sql = 'UPDATE Supplier_details SET ' + field + "= ?" + " WHERE Name=?"
            print(insert_sql)
            pstmt = ibm_db.prepare(conn, insert_sql)
            ibm_db.bind_param(pstmt, 1, value)
            ibm_db.bind_param(pstmt, 2, item)
